...
public final class RouteDocs {
    ...
    private static String fullPath(String leaf) {
        String base = PREFIX.stream().map(s -> s.startsWith("/") ? s : "/" + s).collect(Collectors.joining());
    ...
    private static void addRoute(String method, String path, Role... roles) {
        List<String> names = (roles == null) ? List.of() : Arrays.stream(roles).map(Enum::name).toList();
    ...
    public static Handler overviewHtml = ctx -> {
        ...
        List<RouteEntry> routes = ROUTES.stream().sorted(Comparator.comparing(RouteEntry::path).thenComparing(RouteEntry::method)).toList();
        ...
        String head = """
                ...
                <style>
                  :root {
                    ...
                    --role-recruiter:#ef4444; --role-anyone:#9ca3af;
                    --accent:#6ee7b7;
                  }
                  td a { color: inherit; text-decoration: none; }
                  td a:visited { color: inherit; }
                  td a:hover, td a:focus { text-decoration: none; filter: brightness(1.05); }
                  *{box-sizing:border-box}
                  ...
                  .role.RECRUITER{border-color:var(--role-recruiter);color:var(--role-recruiter)}
...
""";
        ...
        html.append(head.replace("__CTX__", esc(ctxBase)).replace("__COUNT__", String.valueOf(routes.size())));

        for (var r : routes) {
            String fullPathShown = ctxBase + r.path();
            String roleBadges = r.roles().isEmpty() ? "<span class='empty'>—</span>" : r.roles().stream().map(role -> "<span class='role " + esc(role) + "'>" + esc(role) + "</span>").collect(Collectors.joining(" "));
            ...
            html.append("""
                    ...
                    """.formatted(esc(r.path()), esc(r.method()), esc(String.join(",", r.roles())), esc(r.method()), esc(r.method()), esc(fullPathShown),           // link target
